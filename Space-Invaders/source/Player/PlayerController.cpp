...
// a change is made here as it will be used further
...
namespace Player
{
	...
	void PlayerController::processPlayerInput()
	{
		...
		//  to call processbulletfire from left mouse button
	...
	// here, a function named porcessbulletfire created
	void PlayerController::processBulletFire()
	{
		// called soundeffect that was available on the location below 
		ServiceLocator::getInstance()->getSoundService()->playSound(SoundType::BULLET_FIRE);

		// called the current position of the player
		sf::Vector2f currentPosition = player_model->getPlayerPosition();

		// here, called the current position and added the offset of the bullet 
		sf::Vector2f bulletPosition = currentPosition + player_model->barrel_position_offset;

		// defined particle type of the particle which was in model class 
		ParticlesType particleType = ParticlesType::EXPLOSION;

		// here, bullet is spawned. so called that fucntion which was already defined 
